...
class Db extends Options
{
    ...
    public function connect()
    {
        if (empty($this->dbh)) {
            ...
            try {
                ...
                /*
                 * connect['type'] - что-то типа 'mysql:host=myhost;dbname=mydb'
                 * connect['options'] - берется из конфига (base.php)
                 */
                $this->dbh = new \PDO(
     ...
     * @return void
     */
    protected function log($sql, array $context = array())
    {
        $sql = str_replace('%', '%%', $sql);
