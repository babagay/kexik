...
define(['jquery', 'bluz', 'bluz.notify'], function ($, bluz, notify) {
    ...
    $(function () {
        ...
        $('body')
            ...
            .on('click.bluz.ajax', '.load', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        // [!] Сюда заходят аджак-запросы , возвращающие HTML
                        // console.log("HTML")
                        $this.trigger('success.ajax.bluz', arguments);
                        var $target = $(target);
                        if ($target.length === 0) {
                        ...
                        // $target это id элемента, в который запишется полученный html
                        // содержится в атрибуте data-ajax-target
                        // напр, <a class="load " data-ajax-target="#load" href="http://babagay.ru/test/ajax-html">Loading</a>
                        $target.html(data);
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        $this.trigger('error.ajax.bluz', arguments);
            ...
            // Ajax modal dialog
            // <a class="btn btn-default bluz-tooltip dialog" title="" data-toggle="tooltip" href="/test/privilege" data-original-title="Return error dialog">Denied page, dialog</a>
            ...
            .on('submit.bluz.ajax', 'form.ajax', function () {
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus, jqXHR) {
                        //console.log(data) // obj.asd
                        //console.log(textStatus) // success
                        //console.log(jqXHR)
                        // obj.responseJSON.asd
                        // obj.status - 200
                        // obj.statusText - OK

                        // console.log(arguments) // включает data textStatus  jqXHR

                        // Что ожидает этотт метод ?
                        // И что он делает с полученной инфой ?
                        /* require(['bluz.form'], function(form) {
                         form.notices($this, "sdcsdc");
                         }); */
                        $this.trigger('success.ajax.bluz', arguments);
