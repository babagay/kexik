    ...
    $.fn.jSlider = function(options) {
        ...
        return this.each(function() {
			...
			function slider(page, direction, from) {
				...
				if (!options.infinite) {
					if (page == 1) {prev.addClass(options.inactive).css({"cursor":"default"});  prev.addClass(options.inactive).css({"width":"100%"});} else { prev.removeClass(options.inactive).css({"cursor":"pointer"});prev.addClass(options.inactive).css({"width":"100%"}); }
					if (page == number_of_items) { next.addClass(options.inactive).css({"cursor":"default"});  next.addClass(options.inactive).css({"width":"100%"}); } else { next.removeClass(options.inactive).css({"cursor":"pointer"});next.addClass(options.inactive).css({"width":"100%"}); }
				}
			}
			...
			function show(page, from) {
				if (options.images) {
					if (number_of_items != 1) {				
						...
						//obj.innerHTML='<img src="'+img+'" width="100%" >';//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					}
				}
